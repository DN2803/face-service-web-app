...
def user_project():
    ...
    if request.method == 'PATCH':
        ...
        if 'new_project_name' not in data:
            raise Exception('The request is missing the <new_project_name> parameter.')
...
def team_management():
    ...
    if not is_admin: raise Forbidden('Given API Key is not an admin key!')
    ...
    if request.method == 'POST':
        ...
        key_auth_service.check_access(admin_key_obj.id, scope)
    ...
    if request.method == 'PATCH':
        ...
        key_auth_service.check_access(admin_key_obj.id, collection_ids)
